    ...
    data() {
      ...
      return {
        ...
        columns: [
          ...
          { label: '发表时间', prop: 'created_at', align: 'center', display: 'none', options: that.models, formType: 'input' },
          { label: '商品', prop: 'goods', align: 'center', display: 'none', options: that.models, formType: 'input' }
        ],
        filters: [
          {
            name: 'model_id',
        ...
        columnsSchema: {
          '商品': {
            ...
            propsHandler({ col, row }) {
              return { goods_id: row.goods.id, goods_name: row.goods.name }
            },
            component: Vue.extend({
              props: ['goods_id', 'goods_name'],
              render(h) {
                return h('a', {
                  style: {
                    color: '#35a0fc'
                  ...
                  attrs: {
                    href: api.base + '/goods/' + this.goods_id,
                    target: '_blank'
                  }
                }, this.goods_name)
              }
